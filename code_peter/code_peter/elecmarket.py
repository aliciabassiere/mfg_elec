...
class Simulation_Base:
    # main class for both MFG and central planner problem
...
class Simulation_planner(Simulation_Base):
    # main class for performing the MFGsimulation
    def __init__(self, cagents, ragents, cp):
        Simulation_Base.__init__(self,cagents, ragents, cp)
        self.rho = cp['discount rate']
        self.Niter = 0
    def packPr(self,Prp,Prop,fPrice):
        x = np.zeros(self.Nt*(2+self.Nfuels))
        x[:self.Nt] = Prp
        x[self.Nt:2*self.Nt] = Prop
        x[2*self.Nt:] = np.reshape(fPrice,-1)
        return x
    def unpackPr(self,x):
        Prp = x[:self.Nt]
        Prop = x[self.Nt:2*self.Nt]
        fPrice = np.reshape(x[2 * self.Nt:], (self.Nfuels, self.Nt))
        return Prp, Prop, fPrice


    def objective(self,x):
        Prp, Prop, fPrice = self.unpackPr(x)
        obj = 0
        self.Niter = self.Niter+1
        dt = self.T[1]-self.T[0]
        print("Function call",self.Niter)
        for a in self.ragents:
            ob, val, m,mhat,mu,muhat = a.bestResponse(Prp,Prop,self.fTax,fPrice)
            obj = obj + ob + val
            print(ob,val)
            print(a.name,obj)
        for a in self.cagents:
            ...
            print(a.name,obj)
        for i in range(self.Nt):
            obj = obj+np.exp(-self.rho*self.T[i])*dt*(-pcoef*Prp[i]*self.pdemand[i]-opcoef*Prop[i]*self.opdemand[i]+pcoef*G0(Prp[i])+opcoef*G0(Prop[i])+self.psibar(fPrice[:,i]))
        #print("Peak price",Prp)
        #print("Off-peak price",Prop)
        #print("Fuel price",fPrice)
        print(obj)
        #print("Function call",self.Niter,"; objective value: {:.8f}".format(obj))
        return obj
    ...
    def derivative(self,x,h=1e-6):
        ...
        for i in range(len(x)):
            ...
            shift[i] = h
            res[i] = (self.objective(x+shift)-self.objective(x-shift))/2/h
        return res

    def run(self):
        start = time.time()
        self.calcPrice() # Compute equilibrium prices corresponding to the initial measures, as starting point
        x0 = 20*np.ones(self.Nt*(2+self.Nfuels))
        #self.packPr(self.Prp,self.Prop,self.fPrice)
        bds = [(0,Pmax),(0,Pmax)]*self.Nt+[(0,None)]*self.Nfuels*self.Nt
        opres = minimize(self.objective,x0,bounds=bds,method="L-BFGS-B")#options={"maxiter":20000}
        self.Prp,self.Prop,self.fPrice = self.unpackPr(opres.x)
        print('Message:',opres.message)
        obj = 0
        print("Prices after optimization")
        print(self.Prp,self.Prop,self.fPrice)
        for a in self.ragents:
            ob, val, m,mhat,mu,muhat = a.bestResponse(self.Prp,self.Prop,self.fTax,self.fPrice)
            obj = obj + ob + val
            a.update(1,m,mhat,mu,muhat)
            print(a.name,ob,val)
        for a in self.cagents:
            ob, val, m,mhat,mu,muhat = a.bestResponse(self.Prp,self.Prop,self.fTax,self.fPrice)
            obj = obj + ob + val
        ...
        print('Derivative:', self.derivative(opres.x))
        print("Prices after best response")
        self.calcPrice()
        print(self.Prp,self.Prop,self.fPrice)
        return obj, end-start, self.Niter





class Simulation_MFG(Simulation_Base):
    # main class for performing the MFGsimulation
    def __init__(self, cagents, ragents, cp):
        Simulation_Base.__init__(self,cagents, ragents, cp)
