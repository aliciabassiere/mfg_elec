...
import os as os
scenario_name = "test_planner" # this will be the name of the output file & folder
...
cagents = [Conventional('Gas',cp,getpar('gas.py'))]
...
Niter = cp['iterations']
tol = cp['tolerance']
sim = Simulation_planner(cagents,ragents,cp)
conv, elapsed, Nit = sim.run()
...
plt.plot(2018+out['time'], out['Fuel 0'], label='Gas price')
plt.legend()
...
plt.bar(2018+out['time'],out['Gas peak supply'],width=0.25,label='Gas supply')
plt.bar(2018+out['time'],out['Renewable peak supply'],width=0.25,
        bottom=out['Gas peak supply'],label='Renewable supply')
#plt.ylim([0,80])
plt.title('Conventional/ renewable peak supply, GW')
plt.legend()
plt.subplot(122)
plt.bar(2018+out['time'],out['Gas offpeak supply'],width=0.25,label='Gas supply')
plt.bar(2018+out['time'],out['Renewable offpeak supply'],width=0.25,
        bottom=out['Gas offpeak supply'],label='Renewable supply')
